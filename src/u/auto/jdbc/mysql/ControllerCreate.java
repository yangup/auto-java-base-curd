...
public class ControllerCreate {
	 ...
	 * @param modelName
	 */
	public ControllerCreate(Table table) {
		try {
			String modelName = CharUtil.UnderlineToUppercaseTitle(table.getTableName());
			String fileName = null;
			if (Constant.isTable) {
				fileName = Constant.path + modelName + File.separator + modelName + "Ctr.java";
			} else {
				fileName = Constant.path + Constant.package_ctr + File.separator + modelName + "Ctr.java";
			}
			...
			BufferedReader reader = new BufferedReader(new FileReader(Constant.ctr));
