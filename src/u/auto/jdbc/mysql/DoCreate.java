...
public class DoCreate {
	 ...
	 * @param modelName
	 */
	public DoCreate(Table table) {
		try {
			this.list = table.getColumnInfos();
			String modelName = CharUtil.UnderlineToUppercaseTitle(table.getTableName());
			String fileName = null;
			if (Constant.isTable) {
				fileName = Constant.path + modelName + File.separator + modelName + "Do.java";
			} else {
				fileName = Constant.path + Constant.package_do + File.separator + modelName + "Do.java";
			}
			...
			// 读取model模板
			BufferedReader reader = new BufferedReader(new FileReader(Constant.do1));
			...
			for (ColumnInfo columninfo : list) {
				...
				for (int i = 0; i < Constant.fieldType.length; i++) {
					...
					if (Arrays.asList(str).contains(type)) {
						...
						// get-end
						// set-start
						line = new String("\tpublic void " + CharUtil.UnderlineToUppercase("set_" + columninfo.getColumnName()) + "(" + type + " " + field + ") {\n");
						writer.write(line);
